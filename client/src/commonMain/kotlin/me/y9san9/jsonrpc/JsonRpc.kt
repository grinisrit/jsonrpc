 ...
 * (https://www.jsonrpc.org/specification). There are multiple entry points to
 * construct this class and you usually want to use extension functions on the
 * companion object.
 *
 * For example:
 * ```kotlin
 ...
 * being used inside [JsonRpc.Connector.connect] lambda.
 */
...
private constructor(
     ...
     * automatically when [scope] execution is finished. This happens when user
     * code (lamba of rpc.connect) is finished executing and all the jobs
     * launched there are finished.
     *
     * For example, you might log all the events in that scope and this job will
     * not hold you from continuing execution after all other jobs are done.
     *
    ...
    /**
     * Launching jobs in this scope will postpone closing the socket until all
     * the jobs are finished. That means, launching a never-ending update
    ...
    public val incoming: JsonRpcIncoming =
        JsonRpcIncoming(
            rpc = this,
    ...
    /**
     * Unique IDs must be used for each request and you get get one using this
     * method.
     */
     ...
     *
     * If [requests] have more than a single element, it will be executed in a
     * batch request unless [config.batchRequests] is set to false.
     *
    ...
    /**
     * Sends a [response] to the other side according to specification. Response
     * must contain id of request you are responding to. You can't respond to
     * notification.
     */
    ...
    /**
     * Sends a list of [responses] to the other side according to specification.
     * Responses must contain ids of requests you are responding to. You can't
    ...
    public suspend fun respond(responses: List<JsonRpcResponse>) {
        ...
        val json =
            if (config.batchRequests && responses.size > 1) {
                config.json.encodeToString(serializable)
            } else {
                config.json.encodeToString(serializable.first())
            }
    ...
    public sealed interface Result<out T> {
        ...
        public data class Success<out T>(override val value: T) : Result<T>
    }
    ...
    public class Connector(
        ...
        /**
         * Connect to tranport and disconnect as soon as [block] execution is
         * finished. If transport will be disconnected earlier due to external
         * network conditions, [block] execution will be cancelled.
         *
        ...
        public suspend fun <T> connect(
            ...
            val result =
                transport.connect {
                    ...
                    coroutineScope {
                        ...
                        //
                        // There are 3 main reasons why scope is created this
                        // way:
                        //
                        // * I want to pass this scope around and I need it in
                        //  variable, `launch` will not help me there
                        //
                        // * When I cancel background scope, I don't want to
                        // cancel
                        //  [this], so it will be able to return a success value
                        //
                        // * When there is an exception in backgroundScope, I
                        // want
                        //  to propagate it up, so I use [coroutineContex.job]
                        // to
            ...
            return when (result) {
                is JsonRpcTransport.Result.Success ->
                    with(result) { Result.Success(value) }
                is JsonRpcTransport.Result.TransportFailure ->
                    with(result) { Result.TransportFailure(message, cause) }
            }
        }
    }
}
